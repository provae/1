...
def get_token_balance_raw(mint_str: str) -> int:
    """Haalt de raw token balance (met decimals) op van ons wallet voor deze mint via direct RPC call."""
    try:
        mint = Pubkey.from_string(mint_str)
        ...
        if "error" in data:
            logger.warning(f"RPC error bij getTokenAccountsByOwner voor {mint_str}: {data['error']}")
            return 0
        ...
        logger.error(f"Fout bij ophalen token balance voor {mint_str}: {e}")
        return 0
